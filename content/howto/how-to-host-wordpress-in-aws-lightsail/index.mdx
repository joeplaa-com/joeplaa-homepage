...
*How-to written and screenshots taken on March 4 2020*
...
2. [Install LEMP stack](#install-lemp-stack)
3. [Backups](#backups)
...
Using the root account needs to be avoided and instead a daily admin account should be made. I have described how to do that here: [Hosting ASP.NET apps on AWS part 4](hosting-asp-net-apps-aws/part-4-iam-users-and-roles/).

### 2. Setup Lightsail instance

Now that we have our account and have logged in with our admin user, we can go to the [Lightsail dashboard](https://lightsail.aws.amazon.com/). Here we click on the **Create instance** button.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-01.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-01.png' alt='aws lightsail dashboard with the "create instance" button hightlighted' class='img-fluid'></img></a>

I already had an instance running. When first logging in, the dashboard isn't empty though. It shows a big "Create" button.

Now select the region closest to you or your intended visitors to make the latency as low as possible. For me that is `Frankfurt`. Optionally you can also select the availability zone, but for a single instance this doesn't matter.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>

Next we can select the OS (platform), `Linux/Unix`, and the OS type, `Ubuntu 18.04`. If you want an easy installation and your application is listed, you can also select `Apps + OS`. This will use a Bitnami image with everything preinstalled en configured.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-03.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-03.png' alt='Visual selection of the OS (Linux or Windows) and the Linux version (Ubuntu 18.04)' class='img-fluid'></img></a>

Scrolling down further, we can select some optional items. For now we leave all of these settings on the default values. I haven't made a script yet to install the software, that would be awesome. You can change the default keys, but this is only needed when you want to use the same for multiple instances. Or if you really want to use your own.

Also we can select the instance size. WordPress will run fine on all of them. Our intended setup uses more memory, so we need at least the second (1 GB) option.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-04.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-04.png' alt='Set optional launch script, keys and snapshots. Choose instance size.' class='img-fluid'></img></a>

As the last step we enter the instance name. I chose `joeplaa-server-u01` for my server. It hosts the `joeplaa` website, it is a web`server` and is `U`buntu instance number `01` (`02` for this how-to).

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-05.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-05.png' alt='Set the instance name to joeplaa-server-u02' class='img-fluid'></img></a>

After we click the **Create instance** button the instance will be initiated. This can take a few minutes. After that the instance will be accessible through the dashboard as shown in the first picture.

### 3. Setup Route 53

I wanted to move my domain to AWS. This is not necessary. You can also use any other registrar, as long as you can configure your DNS and nameserver settings. In the first part of [Hosting ASP.NET apps on AWS part 7](hosting-asp-net-apps-aws/hosting-asp-net-apps-aws-part-7-domains-and-dns-with-route-53/) I describe how to register and/or transfer a domain to Route 53. You can stop when you arrive at the "DNS management" heading. We'll do that in Lightsail.

### 4. Setup Lightsail networking

In the Lightsail dashboard go to the **Networking** tab. Now click the **Create static IP** button.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-06.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-06.png' alt='The networking tab of the Lightsail dashboard. Showing DNS zone and static IP address.' class='img-fluid'></img></a>

Select the region. Use the same as your instance. I choose `Frankfurt`.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-07.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-07.png' alt='Flags showing the available regions for the IP address' class='img-fluid'></img></a>

Next we select the instance (I can't as all my instances already have a static IP attached). And we give it a name. I named mine `joeplaa-static-ip1`. **Create** the IP.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-08.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-08.png' alt='Attach and name the IP address' class='img-fluid'></img></a>

With the IP address created and the nameservers of the domain accessible, we now click **Create DNS zone**.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-09.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-09.png' alt='Lightsail networking dashboard' class='img-fluid'></img></a>

Enter the domain, for me that's `joeplaa.com` and click **Create DNS zone** at the bottom of the page.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-10.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-10.png' alt='Entered domain name on create DNS zone page' class='img-fluid'></img></a>
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-11.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-11.png' alt='Censored nameservers for joeplaa.com DNS zone' class='img-fluid'></img></a>

Something I didn't understand at first is where and how to configure the nameservers in Route 53. Go to [your registered domains](https://console.aws.amazon.com/route53/home#DomainListing:) in the Route 53 dashboard. Open the domain for this DNS zone.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-12.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-12.png' alt='Details for the domain. Edit nameservers and disable DNSSEC' class='img-fluid'></img></a>
...
> [https://linuxize.com/post/how-to-setup-a-firewall-with-ufw-on-ubuntu-18-04](https://linuxize.com/post/how-to-setup-a-firewall-with-ufw-on-ubuntu-18-04/)
...
6. Check with the following command:
    ...
    <a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-55.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-55.png' alt='enabled ufw rules ufw' class='img-fluid'></img></a>

7. Now go to the Lightsail dashboard, click on your instance and select the **Networking** tab. Here you'll see another firewall. This is the firewall AWS provides. It sounds like doing the firewall in Linux is double the work and unnecessary, but better be safe and enable both. Again the ports `22`, `80` and `443` need to be open.

    <a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-28.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-28.png' alt='Networking tab of the instance settings. It shows public and private IP addresses, opened ports in firewall and load balancing' class='img-fluid'></img></a>

### 3. Enable Swap

Preferably you only use the RAM of your instance, because that is the fastest option to store data. As we only have 1 GB, this is quite tight. So the second option would be to create swap space on the local SSD, ephemeral storage in AWS terms. A Lightsail instance however doesn't have that, so we are forced to forgo on best-practices and use the attached network volume.

AWS best practices regarding swap space: [https://aws.amazon.com/premiumsupport/knowledge-center/ec2-memory-swap-file/](https://aws.amazon.com/premiumsupport/knowledge-center/ec2-memory-swap-file/)
...
7. Add the following new line at the end of the file (use arrow keys and paste the line with right-mouse button), save the file (`ctrl+x`), and then exit (`y` and hit `Enter`):
    ...
    <a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-54.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-54.png' alt='add line to /etc/fstab using nano editor' class='img-fluid'></img></a>
...
8. When starting a new SSH session, Ubuntu will display the amount of used memory, but you can also see it by using top or htop. These can be opened by simply typing `top` or `htop` in the terminal.
    ...
    <a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-52.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-52.png' alt='memory info at start of session' class='img-fluid'></img></a>

    Memory usage info when starting an SSH session

    <a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-53.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/ec2-53.png' alt='top information' class='img-fluid'></img></a>

    System information and memory usage as displayed in top. Sort on memory (%MEM): `shift+M`

### 4. Install Redis, Nginx, PHP, MySQL

For [jodiBooks](https://jodibooks.com) I configured the exact same stack. The only difference is that it was done on an EC2 instance instead of Lightsail. The process however is (almost) exactly the same (I used that guide to install this blog). The only difference is the S3 plugin, which I'll discuss below.

https://blog.jodibooks.com/2020/01/hosting-asp-net-apps-aws-part-13-wordpress-hosting-on-lemp-ssl-s3/
...
`/var/www/<website>/wp-content`/

`www-data:www-data`

`755`

`/var/www/<website>/wp-config.php`
...
Source: [https://stackoverflow.com/questions/18352682/correct-file-permissions-for-wordpress#comment82099892\_23755604](https://stackoverflow.com/questions/18352682/correct-file-permissions-for-wordpress#comment82099892_23755604)
* * *
...
Now that we have a working WordPress website, it'a good idea to make a backup of all the work we did. If anything goes wrong in the future, be it a hack or you yourself (accidentally) screwing something up, you can go back to this state. This is only valid for all the software you installed on the instance, not the actual data you add to your website (copy, posts, images). We'll backup those separately.
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![The Lightsail dashboard with a visual list of instances. In this case the instance joeplaa-server-u01.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-13.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-13.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![The snapshot overview page with one manual snapshot and automatic snapshot disabled..](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-14.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-14.png)

3. Enter a name for your snapshot. Personally I want to know what instance this snapshot belongs to `joeplaa-server-u01`, the date `20200503` and why `initial setup` or `OS update` or something. I forgot with my first snapshot though. Press **Create**.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Give the new snapshot a name. Something recognizable.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-15.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-15.png)
* * *
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![The S3 dashboard with two buckets](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-16.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-16.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Create bucket display. We've entered the bucket name and chosen the region (Frankfurt).](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-17.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-17.png)

3. Click **Next** and select _Versioning_ and _Default encryption_: **AES-256**.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Screen with multiple bucket config options. We only enable Versioning and Encryption.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-18.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-18.png)

4. In the next screen keep the setting: **Block _all_ public access** and click **Next**.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Block public access settings](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-19.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-19.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![User list in AWS IAM dashboard showing 3 users.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-20.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-20.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Add user page in IAM panel. Added a user name and selected Programmatic access as the Access type](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-21.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-21.png)

8. In the next screen click **Attach existing policies directly** and then click the **Create policy** button.

<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Set user permissions. Select a policy from the list or create one.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-22.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-22.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Add JSON to describe the policy](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-23.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-23.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Review policy screen with entered name and description.](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-24.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-24.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Select the policy you just created](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-25.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-25.png)

12. Click **Next** twice and **Create user**. Now save the credentials somewhere safe.
13. Open the Updraft plugin settings in WordPress. They can be found in the Settings part of the main menu.
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![The Updraft settings screen. Set schedule to daily and retain 14 backups. Also selected Amazon S3 as storage](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-26.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-26.png)
...
<a href='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png'><img src='/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-02.png' alt='a visual list (flags, with country, state or city) of all the available regions' class='img-fluid'></img></a>
[![Add S3 bucket name and credentials and set folders to backup and exclude](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-27.png)](/assets/howto/how-to-host-wordpress-in-aws-lightsail/WP-AWS-Lightsail-27.png)
* * *
