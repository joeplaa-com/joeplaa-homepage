...
[https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/](https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/)
...
1. Login to the Ubuntu EC2 instance with PuTTY as described in [part 6](https://blog.jodibooks.com/2020/02/hosting-asp-net-apps-aws-part-6-servers-in-ec2-part-2/).
...
[![add A type DNS record Route 53](images/wp-01-1024x554.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-01.png)
...
[![nginx 404 not found](images/wp-02.png)](https://blog.jodibooks.com/wp-content/uploads/2020/02/wp-02.png)
...
1\. [https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/](https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/)  
2\. [https://www.lisenet.com/2015/optimise-mysql-configuration-for-wordpress/](https://www.lisenet.com/2015/optimise-mysql-configuration-for-wordpress/)
...
6. Now we're going to create a hidden file with our credentials, so we don't have to enter them every time ([https://easyengine.io/tutorials/mysql/mycnf-preference/](https://easyengine.io/tutorials/mysql/mycnf-preference/)). Open the file in the nano editor.
...
[![results tuning-primer script](images/wp-03-1024x782.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-03.png)

The results for our server after 19 days of continuous uptime. Our settings seem to be fine.
...
[![php processes in top](images/wp-04.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-04.png)

### Fail2ban

To quote Dave Hilditch: "_The basic install, if you’ve followed the installation above, automatically includes SSH/putty attacks and blocks those attacks based on IP addresses._  
_I will write a separate article about configuring fail2ban as it can be complicated, but if you wish to get this set up, you should install the [WP fail2ban plugin](https://wordpress.org/plugins/wp-fail2ban/) and follow their guide for adding their ‘jails’ and ‘filters’. Basically, fail2ban uses filter config files to spot dodgy traffic and then uses the jail config files to decide how long to ban them._"

I'm could find that article and haven't bothered since. So please share if you have a decent tutorial on fail2ban.
...
"_For some weird reason, the WordPress installer fails miserably if your site starts out HTTPS. So, you have to install over HTTP and then convert to HTTPS._" As I haven't done a new install, I'll advise you to head over to the original post here: [https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/](https://www.wpintense.com/2018/10/20/installing-the-fastest-wordpress-stack-ubuntu-18-mysql-8/) and browse to the "Changing your site to use SSL" section.
...
[![duplicator plugins wordpress](images/wp-05-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-05.png)

2. Go to the plugin page and click **Create New**.

[![create new duplicator package](images/wp-06-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-06.png)

3. Give the package a _name_ `20200219_jodibooks` and click **Next**.

[![setup package](images/wp-07-1024x554.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-07.png)

4. After scanning we find a notice on big files. Nothing much to do about it, so we mark the checkbox and press **Build**.

[![build the package](images/wp-08-1024x555.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-08.png)

5. Give it some time. When the build is done, download both files to your local computer.

[![download package files](images/wp-09-1024x555.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-09.png)
...
[![pscp in putty folder](images/wp-10.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-10.png)
...
[![copy the files using pscp and the result](images/wp-11.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-11.png)
...
[![run installer in browser](images/wp-12-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-12.png)

13. I can't continue any further, because I don't want to overwrite my existing installation. In the next page you have to enter your MySQL host, database name, user and password. The host is `localhost`. After that it is all pretty straight forward. If in doubt check [step 6 here](https://www.wpbeginner.com/wp-tutorials/how-to-move-wordpress-to-a-new-host-or-server-with-no-downtime/#runduplicatorscript).
...
[![open the wordpress ec2 role](images/wp-12-1-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-12-1.png)

6. Now **Attach policies** and **Create policy**.

[![attach policies](images/wp-13-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-13.png)

7. In the newly opened tab click **JSON** and paste the code below. Change the _Resource_ to your own bucket. [https://deliciousbrains.com/wp-offload-media/doc/custom-iam-policy-for-amazon-s3/](https://deliciousbrains.com/wp-offload-media/doc/custom-iam-policy-for-amazon-s3/)
...
[![set json policy](images/wp-15-1024x554.png)](https://s3-eu-central-1.amazonaws.com/jodibooks-public-cdn/blog/wp-content/uploads/2020/01/19183622/wp-15-1.png)
...
[![attach the policy to the role](images/wp-16-1024x554.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-16.png)

11. Open the plugin and select **S3** as the storage provider. Select to use **IAM Roles**.

[![select s3 as storage provider](images/wp-17-1024x553.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-17.png)

12. Click **Browse existing buckets** and select the bucket `jodibooks-public-cdn`.

[![select the bucket form the list](images/wp-18-1024x554.png)](https://blog.jodibooks.com/wp-content/uploads/2020/01/wp-18.png)
...
With that we have a fully working environment. What we still need to do is make sure everything will be backed up and monitored. That will be the topic of the [next parts](https://blog.jodibooks.com/2020/01/hosting-asp-net-apps-aws-part-14-monitoring-with-cloudwatch/).
