...
Currently we run one instance of each "machine". Every instance has a "maximum" number of users and visitors it can serve, before the average CPU usage gets [bigger than the baseline](https://blog.jodibooks.com/2020/02/hosting-asp-net-apps-on-aws-appendix-d-ec2-instance-type-selection/). When we get more users than one instance can support, their experience will suffer, by longer load times or increased latencies.

[![current architecture](images/AWS-architecture-jodiBooks-current.png)](https://blog.jodibooks.com/wp-content/uploads/2020/02/AWS-architecture-jodiBooks-current.png)
...
This database instance can be a self-managed EC2 instance or a managed [RDS](https://aws.amazon.com/rds/) instance. The latter is preferred obviously. However, SQL server RDS instances are [pricey](https://blog.jodibooks.com/2020/01/hosting-asp-net-apps-aws-appendix-b-aws-pricing/), so we also want to migrate the application database to MySQL. With that we can move both databases (application and WordPress) to the [MySQL RDS instance](https://aws.amazon.com/rds/mysql/).
...
[![first improvement, managed rds](images/AWS-architecture-jodiBooks-RDS-S-AZ.png)](https://blog.jodibooks.com/wp-content/uploads/2020/02/AWS-architecture-jodiBooks-RDS-S-AZ.png)
...
[![Architecture with load balancer and auto scaling group](images/AWS-architecture-jodiBooks-RDS-S-AZ-ALB-1024x744.png)](https://s3-eu-central-1.amazonaws.com/jodibooks-public-cdn/blog/wp-content/uploads/2020/02/23170744/AWS-architecture-jodiBooks-RDS-S-AZ-ALB.png)
