...
Our WordPress blog will be hosted on [a separate Ubuntu instance](../hosting-asp-net-apps-aws-part-6-servers-in-ec2-part-2/) with a [LEMP stack](https://lemp.io/). We tried running it in IIS on Windows as well, but we needed the memory for our ASP.NET applications (and WordPress is much faster on Linux).
