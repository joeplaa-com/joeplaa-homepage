...
If something is wrong with a TrueNas service or the disks, TrueNas can send you an email. This has to be configured obviously. If you don't want to use Gmail, but rather run your own relay, check [How to run a an AWS SMTP relay on Proxmox](/how-to-run-aws-smtp-relay-in-lxc-on-proxmox).
