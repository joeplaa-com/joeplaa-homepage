...
LXC containers are light-weight alternatives to Qemu guests. They can be used to run a single application for example. See [How to run a Docker in LXC container on ZFS on Proxmox](/how-to-run-docker-in-lxc-on-zfs-on-proxmox), [How to run a LXC container with snap on Proxmox](/how-to-run-snap-in-lxc-on-proxmox) and [...]() for examples.
