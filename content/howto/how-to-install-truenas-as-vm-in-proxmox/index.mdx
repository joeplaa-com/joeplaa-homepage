...
9. When installation is done reboot the system.
    ...
    <Alert type='warning'>
        Although the installer asks to remove the installation medium, do not do this. After removing, the reboot "button" does not work anymore.
    </Alert>
...
3. Start the VM. The HBA will be booted inside the VM.
    ...
    <Alert type='warning'>
        Initially when I added the PCIe HBA, TrueNas inserted the drives attached to the HBA in front of the OS disk. During boot the VM wanted to boot from a HBA drive instead of the virtual OS drive. After disabling the boot option of the HBA TrueNas booted fine. To disable the boot option on a LSI card, you have to erase the BIOS, see <a href='/how-to-flash-an-ibm-m1115-hba-to-it-mode'>How to flash an IBM M1115 HBA card</a>.
    </Alert>
